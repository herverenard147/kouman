        ...
        <ul class="buy-button list-none mb-0">
            ...
            <li class="dropdown inline-block relative">
                ...
                <div class="dropdown-menu absolute left-1/2 -translate-x-1/2 m-0 mt-4 z-10 w-44 rounded-md overflow-hidden bg-white dark:bg-slate-900 shadow hidden" onclick="event.stopPropagation();">
                    <ul class="py-2 text-center text-slate-900 dark:text-white">
                        <li>
                        ...
                        <li>
                            <form action="{{ route('client.logout') }}" method="POST">
                                @csrf
                                <button type="submit" class="block w-full text-center py-1 px-4 hover:text-green-600 bg-transparent border-0">
                                    Déconnexion
                                </button>
                            </form>
                        </li>
            ...
            {{-- Icône Commandes pour partenaire --}}
            <li class="inline mb-0">
                <a href="{{ route('partenaire.orders.index') }}"
                    class="btn btn-icon bg-yellow-500 hover:bg-yellow-600 border-yellow-500 dark:border-yellow-500 text-white rounded-full relative">
                    {{-- Icône liste / commandes --}}
                    ...
                    @php
                    $orderCount = \App\Models\Order::whereHas('products', function($q) use ($user) {
                    $q->where('partenaire_id', $user->id);
                    })->count();
                    @endphp
